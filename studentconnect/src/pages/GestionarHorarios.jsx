...
const GestionarHorarios = () => {
    ...
    useEffect(() => {
        horarioService.obtenerMisHorarios()
        ...
        <>
            <div className="gestionar-horarios-container">
               ...
               <div style={{ marginTop: '20px' }}>
                {horarios.length === 0 ? (
                    <p>No has publicado asesorías todavía.</p>
                ) : (
                    horarios.map(horario => (
                    <div key={horario.id} style={{ border: '1px solid #ccc', padding: '10px', marginBottom: '10px', borderRadius: '8px' }}>
                        <h3>{horario.titulo_asesoria}</h3>
                        <p>Inicio: {new Date(horario.fecha_hora_inicio).toLocaleString()}</p>
                        <p>Fin: {new Date(horario.fecha_hora_fin).toLocaleString()}</p>
                        <p>Modalidad: {horario.modalidad}</p>
                        <p>Estado: {horario.estado_disponibilidad}</p>
                        </div>
                        ))
                        )}
                        </div>
